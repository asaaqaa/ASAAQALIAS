    ...
    try:
        if BOTLOG:
            Config.ZEDUBLOGO = await zedub.tgbot.send_file(
                BOTLOG_CHATID,
                "https://graph.org/file/ba2e844191a98947e5183.jpg",
                caption="**•⎆┊تـم بـدء تشغـيل سـورس الامبراطـــور 𓅛 الخاص بك .. بنجاح 🧸♥️**",
                buttons=[(Button.url("الامبراطـــور 𓅛", "https://t.me/kapo00s"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", "AsAs")
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"AsAs/{folder}/*.py"
        plugin_path = f"AsAs/{folder}"
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "كـروب السجـل الامبراطـــور 𓅛", zedub, Config.TG_BOT_USERNAME, descript, photozed
        )
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    await load_plugins(folder="AsAs", extfolder=cfolder)
