...
from AsAs import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"AsAs/plugins/{shortname}.py")
        checkplugins(path)
        name = "AsAs.plugins.{}".format(shortname)
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"AsAs.plugins.{shortname}"] = mod
        LOGS.info(f"تـم تثبيت ملـف {shortname}")
