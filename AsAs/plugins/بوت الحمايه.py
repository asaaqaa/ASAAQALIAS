...
# All Rights Reserved for "asaaqaa - ASAAQALIAS" "الإمبراطور  الإمبراطور "
...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "اوامـر ذكيـه لـ قفـل / فتـح حمـاية المجمـوعـات بالمسـح والطـرد والتقييـد لـ اول مـره فقـط ع سـورس 𝞝 الإمبراطور 𝞝",
        "الاوامـر": {
...
async def _(event):
    ...
    # All Rights Reserved for " asaaqaa - ASAAQALIAS " "زلـزال  الإمبراطور "
...
@zedub.zed_cmd(
    ...
    info={
        "header": "لـ عـرض اعـدادات حمـاية المجمـوعـة الخـاصـه ببـوت 𝞝 الإمبراطور 𝞝",
...
# Copyright (C) 2022 asaaqaa - كــود قفــل دخــول الايــران
@zedub.on(events.ChatAction())
async def _(event):
    if not event.is_private:
        chat = await event.get_chat()
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "Zed-Thon - zthon" "زلـزال  الإمبراطور "
...
async def zelzal(zed):
    ...
    if con != "طرد":
        ...
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𝞝 الإمبراطور 𝞝\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𝞝 الإمبراطور 𝞝\
